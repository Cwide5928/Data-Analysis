...
590개 공정 센서로 웨이퍼 합불을 예측하고, 시각 탐색(Spotfire)과 머신러닝(Python)을 교차검증해 불량 연관 센서를 도출한 프로젝트. 극단적 클래스 불균형·결측 처리와 SPC 관점 해석을 다룸.
...
[2] Python 머신러닝 — "어떤 센서가 원인인가" 전처리(결측 50%↑·상수 센서 제거, 중앙값 대치, 표준화, 층화 분할) → Pipeline으로 누수 방지 → 기준 모델(LogReg) vs Random Forest → precision-recall 곡선으로 임계값 조정 → Feature Importance.
